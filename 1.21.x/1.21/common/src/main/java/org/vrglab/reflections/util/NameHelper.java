...
public interface NameHelper {
    ...
    List<String> PRIMITIVE_NAMES = Arrays.asList("boolean", "char", "byte", "short", "int", "long", "float", "double", "void");
    List<Class<?>> PRIMITIVE_TYPES = Arrays.asList(boolean.class, char.class, byte.class, short.class, int.class, long.class, float.class, double.class, void.class);
    List<String> PRIMITIVE_DESCRIPTORS = Arrays.asList("Z", "C", "B", "S", "I", "J", "F", "D", "V");
    ...
    default String toName(Class<?> type) {
        int dim = 0;
        while (type.isArray()) {
            dim++; type = type.getComponentType();
        }
    ...
    default Class<?> forClass(String typeName, ClassLoader... loaders) {
        if (PRIMITIVE_NAMES.contains(typeName)) {
            return PRIMITIVE_TYPES.get(PRIMITIVE_NAMES.indexOf(typeName));
        } else {
            String type;
            if (typeName.contains("[")) {
                ...
                if (PRIMITIVE_NAMES.contains(type)) {
                    type = PRIMITIVE_DESCRIPTORS.get(PRIMITIVE_NAMES.indexOf(type));
                } else {
                    type = "L" + type + ";";
                }
                type = array + type;
