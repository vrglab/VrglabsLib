...
import org.Vrglab.azure.azurelib.common.cache.object.GeoQuad;
import org.Vrglab.azure.azurelib.common.cache.object.GeoVertex;
import org.Vrglab.azure.azurelib.common.model.AzBone;
import org.Vrglab.azure.azurelib.common.render.item.AzItemRendererPipelineContext;
...
public class AzModelRenderer<K, T> {
    ...
    public VertexConsumer getOrRefreshBufferRenderType(
        ...
        return switch (currentBuffer) {
            ...
            case VertexMultiConsumer.Double pair when needsBufferRefresh(ReflectionUtil.getField(pair, "first", VertexConsumer.class)) || needsBufferRefresh(
                    ReflectionUtil.getField(pair, "first", VertexConsumer.class)
            ) ->
                new VertexMultiConsumer.Double(
                    needsBufferRefresh(ReflectionUtil.getField(pair, "first", VertexConsumer.class)) ? bufferSource.getBuffer(renderType) : ReflectionUtil.getField(pair, "first", VertexConsumer.class),
                    needsBufferRefresh(ReflectionUtil.getField(pair, "second", VertexConsumer.class)) ? bufferSource.getBuffer(renderType) : ReflectionUtil.getField(pair, "second", VertexConsumer.class)
                );
    ...
    public VertexConsumer getOrRefreshRenderBuffer(
        ...
        return switch (currentBuffer) {
            ...
            case OutlineBufferSource.EntityOutlineGenerator outline when needsBufferRefresh(outline.delegate()) ->
                new OutlineBufferSource.EntityOutlineGenerator(bufferSource.getBuffer(renderType), outline.color());
            case VertexMultiConsumer.Double pair when needsBufferRefresh(ReflectionUtil.getField(pair, "first", VertexConsumer.class)) || needsBufferRefresh(
                    ReflectionUtil.getField(pair, "second", VertexConsumer.class)
            ) ->
                new VertexMultiConsumer.Double(
                    needsBufferRefresh(ReflectionUtil.getField(pair, "first", VertexConsumer.class)) ? bufferSource.getBuffer(renderType) : ReflectionUtil.getField(pair, "first", VertexConsumer.class),
                    needsBufferRefresh(ReflectionUtil.getField(pair, "second", VertexConsumer.class)) ? bufferSource.getBuffer(renderType) : ReflectionUtil.getField(pair, "second", VertexConsumer.class)
    ...
    protected boolean needsBufferRefresh(VertexConsumer buffer) {
        return switch (buffer) {
            ...
            case VertexMultiConsumer.Double pair ->
                needsBufferRefresh(ReflectionUtil.getField(pair, "first", VertexConsumer.class)) || needsBufferRefresh(ReflectionUtil.getField(pair, "second", VertexConsumer.class));
            default -> false;
        };
    }
