...
import org.Vrglab.azure.azurelib.common.cache.texture.AnimatableTexture;
import org.Vrglab.azure.azurelib.common.model.AzBone;
import org.Vrglab.azure.azurelib.common.render.AzLayerRenderer;
import org.Vrglab.azure.azurelib.common.render.AzModelRenderer;
import org.Vrglab.azure.azurelib.common.render.AzRendererConfig;
...
import org.Vrglab.azure.azurelib.common.render.armor.compat.ShoulderSurfingCompat;
...
public class AzArmorRendererPipeline extends AzRendererPipeline<UUID, ItemStack> {
    ...
    public void scaleModelForBaby(AzArmorRendererPipelineContext context, boolean isReRender) {
        ...
        if (currentSlot == EquipmentSlot.HEAD) {
            boolean scaleHead = ReflectionUtil.getField(baseModel, "scaleHead", boolean.class);
            float babyHeadScale = ReflectionUtil.getField(baseModel, "babyHeadScale", float.class);
            float babyYHeadOffset = ReflectionUtil.getField(baseModel, "babyYHeadOffset", float.class);
            float babyZHeadOffset = ReflectionUtil.getField(baseModel, "babyZHeadOffset", float.class);

            if (scaleHead) {
                float headScale = 1.5f / babyHeadScale;

                poseStack.scale(headScale, headScale, headScale);
            }

            poseStack.translate(0, babyYHeadOffset / 16f, babyZHeadOffset / 16f);
        } else {
            float babyBodyScale = ReflectionUtil.getField(baseModel, "babyBodyScale", float.class);
            float bodyYOffset = ReflectionUtil.getField(baseModel, "bodyYOffset", float.class);

            float bodyScale = 1 / babyBodyScale;

            poseStack.scale(bodyScale, bodyScale, bodyScale);
            poseStack.translate(0, bodyYOffset / 16f, 0);
        }
    }
